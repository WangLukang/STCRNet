...
def consistency_loss1(pred1, pred2, weight=1.0):
    ''' MSE loss between two predictions'''
...
def consistency_loss2(pred1, pred2, weight=1.0):
    ''' 余弦相似度损失'''
    cos_sim = nn.CosineSimilarity(dim=1, eps=1e-6)
    sim = cos_sim(pred1, pred2)
    loss = weight * (1 - sim.mean())

    return loss

def consistency_loss3(pred1, pred2, weight=1.0):
    '''L2+余弦相似度'''
...
def main():
    ...
    '''LEVIR dataset'''
    data_dir = 'C:/Users/11473/OneDrive/桌面/semiURNET/dataset/LEVIR/LEVIR/10/'

    tra_image_dirA = 'self_training/A/'
    tra_image_dirB = 'self_training/B/'
    tra_label_dir = 'self_training/label/'

    tra_image_dirA2 = 'train_st1AUG_rest/A/'
    tra_image_dirB2 = 'train_st1AUG_rest/B/'
    tra_label_dir2 = 'train_st1AUG_rest/label/'
    ...
    model_dir = "epochs/LEVIR/self_training/self_training/10_self_training_1_consistency/"
    sta_dir = "statistics/LEVIR10_self_training_1_consistency.csv"
    # model_dir_con = ""
    ...
    tra_img_name_listA2 = glob.glob(data_dir + tra_image_dirA2 + '*' + image_ext)
    tra_img_name_listB2 = glob.glob(data_dir + tra_image_dirB2 + '*' + image_ext)
    tra_lbl_name_list2 = glob.glob(data_dir + tra_label_dir2 + '*' + image_ext)

    print("---")
    print("train imagesA: ", len(tra_img_name_listA))
    print("train imagesB: ", len(tra_img_name_listB))
    print("train labels: ", len(tra_lbl_name_list))
    print('train imagesA2: ', len(tra_img_name_listA2))
    print('train imagesB2: ', len(tra_img_name_listB2))
    print("---")
    ...
    salobj_dataset2 = SalCDDataset(
        img_name_listA=tra_img_name_listA2,
        img_name_listB=tra_img_name_listB2,
        lbl_name_list=tra_lbl_name_list2,
        transform=transforms.Compose([
            RescaleT(256),
            RandomCrop(224),
            ToTensorLab(flag=0)]))
    salobj_dataloader2 = DataLoader(salobj_dataset2, batch_size=batch_size_train, shuffle=True, num_workers=0, drop_last=True)
    ...
    for epoch in range(1, epoch_num+1):
        # loader = zip(cycle(salobj_dataloader), salobj_dataloader2)
        if len(salobj_dataloader) > len(salobj_dataloader2):
            loader = zip(salobj_dataloader, cycle(salobj_dataloader2))
        else:
            loader = zip(cycle(salobj_dataloader), salobj_dataloader2)
        ...
        for i, (data1, data2) in enumerate(loader):
            ...
            inputsA, inputsB, labels = data1['imageA'],data1['imageB'], data1['label']
            inputsA2, inputsB2 = data2['imageA'],data2['imageB']

            inputsA = inputsA.type(torch.FloatTensor)
            inputsB = inputsB.type(torch.FloatTensor)
            labels = labels.type(torch.FloatTensor)
            inputsA2 = inputsA2.type(torch.FloatTensor)
            inputsB2 = inputsB2.type(torch.FloatTensor)

            # print(inputsA.shape, inputsB.shape, labels.shape, inputsA2.shape, inputsB2.shape)
            ...
            if torch.cuda.is_available():
                ...
                inputs_vA2,inputs_vB2 = Variable(inputsA2.cuda(), requires_grad=False), Variable(inputsB2.cuda(), requires_grad=False)
            else:
                ...
                inputs_vA2,inputs_vB2 = Variable(inputsA2, requires_grad=False), Variable(inputsB2, requires_grad=False)
            ...
            loss1 = muti_bce_loss_fusion(d0, labels_v)


            # brightness = np.random.uniform(0.5, 1.5)
            # contrast = np.random.uniform(0.4, 1)
            ...
            inputs_vA2_strong = strong_transform(inputs_vA2)
            inputs_vB2_strong = strong_transform(inputs_vB2)

            with torch.no_grad():
                _,d1 = net(inputs_vA2, inputs_vB2)
            _,d1_ = net(inputs_vA2_strong, inputs_vB2_strong)
            loss2 = consistency_loss3(d1, d1_)
            loss = loss1 + loss2
            

            
            for cc in range(data1['imageA'].shape[0]):
